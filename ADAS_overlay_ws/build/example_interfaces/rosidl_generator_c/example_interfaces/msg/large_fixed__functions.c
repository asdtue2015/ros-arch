...
#include "example_interfaces/msg/large_fixed__functions.h"
...
{
  example_interfaces__msg__LargeFixed * msg = malloc(sizeof(example_interfaces__msg__LargeFixed));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(example_interfaces__msg__LargeFixed));
  bool success = example_interfaces__msg__LargeFixed__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
...
{
  if (msg) {
    example_interfaces__msg__LargeFixed__fini(msg);
  }
  free(msg);
}
...
example_interfaces__msg__LargeFixed__Array__init(example_interfaces__msg__LargeFixed__Array * array, size_t size)
{
  ...
  example_interfaces__msg__LargeFixed * data = NULL;
  if (size) {
    data = malloc(sizeof(example_interfaces__msg__LargeFixed) * size);
    if (!data) {
      return false;
    }
    ...
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = example_interfaces__msg__LargeFixed__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      ...
      for (; i > 0; --i) {
        example_interfaces__msg__LargeFixed__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
...
example_interfaces__msg__LargeFixed__Array__fini(example_interfaces__msg__LargeFixed__Array * array)
{
  ...
  if (array->data) {
    ...
    for (size_t i = 0; i < array->capacity; ++i) {
      example_interfaces__msg__LargeFixed__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
...
{
  example_interfaces__msg__LargeFixed__Array * array = malloc(sizeof(example_interfaces__msg__LargeFixed__Array));
  if (!array) {
    return NULL;
  }
  bool success = example_interfaces__msg__LargeFixed__Array__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
...
{
  if (array) {
    example_interfaces__msg__LargeFixed__Array__fini(array);
  }
  free(array);
}
